...
describe('Statement', () => 
{
	...
	it('GetLineType_Tab', () => 
	{
		const text_tab = "2 PY	4"
		const text =     "2 PY  4"

		const line_number = 10;
		const position = 55;
		var line = StringToMCNPLines(text_tab,line_number,position);

		var statement = new st.Statement(line,null);

		expect(statement.Arguments.length).to.equal(3);
		expect(statement.RawText).to.equal(text_tab);
		expect(statement.StartIndex).to.equal(position);
		
		expect(statement.Arguments[statement.Arguments.length-1].Contents).to.equal("4");
		expect(statement.Arguments[statement.Arguments.length-1].FilePosition).to.equal(6+position);
